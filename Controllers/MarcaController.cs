...
namespace mototek.Controllers
{
    ...
    public class MarcaController : ControllerBase
    {
        ...
        public IActionResult Get(int id)
        {
            ...
            try
            {
                using (DB_A6ED12_testmototekDBContext db = new DB_A6ED12_testmototekDBContext())
                {
                    ...
                    Marca data = db.Marcas.FromSqlRaw("Select * from Marcas where IdMarca = @Id", idSearch)
                        .FirstOrDefault();
                    resp.status = "Ok";
                    resp.message = "Success";
                    resp.data = data;
        ...
        // POST api/Marca
        [HttpPost]
        public IActionResult Post([FromBody] Marca value)
        {
            Respuesta resp = new Respuesta();
            resp.status = "Error";
            resp.data = null;
            try
            {
                using (DB_A6ED12_testmototekDBContext db = new DB_A6ED12_testmototekDBContext())
                {
                    ...
                    {
                        new SqlParameter("@MARCAS", value.IdMarca),
                        new SqlParameter("@NOMBREDEMARCA", value.NombreDeMarca),
                    ...
                    var data = db.Database.ExecuteSqlRaw("INSERT INTO [dbo].[MARCAS] ([NombreDeMarca],[FechaDeCreacion],[FechaDeModificacion],[Descripcion]) VALUES (@NOMBREDEMARCA, @FECHADECREACION, @FECHADEMODIFICACION, @DESCRIPCION)", sqlParams);
            ...
            try
            {
                using (DB_A6ED12_testmototekDBContext db = new DB_A6ED12_testmototekDBContext())
                {
                    SqlParameter[] sqlParams = new SqlParameter[]
                    {
                        ...
                        new SqlParameter("@FIELDTOCHECK", "IdMarca"),
            ...
            try
            {
                using (DB_A6ED12_testmototekDBContext db = new DB_A6ED12_testmototekDBContext())
                {
                    SqlParameter[] sqlParams = new SqlParameter[]
                    {
                        new SqlParameter("@ID", id),
                        new SqlParameter("@TABLE", "MARCAS"),
                        new SqlParameter("@FIELD", "IdMarca"),
                    };
